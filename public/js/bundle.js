...
const showAlert = (type, message) => {
  hideAlert();
  const markup = "<div class=\"alert alert--".concat(type, "\">").concat(message, "</div>");
...
const bookTour = async tourId => {
  try {
    ...
    const session = await (0, _axios.default)("http://localhost:3000/api/v1/bookings/checkout-session/".concat(tourId));
    console.log(session);
